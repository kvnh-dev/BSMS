...
interface PurchaseOrderDetail {
  ...
  purchaseBill: { id: string; billNumber: string | null } | null;
}
...
export default function PurchaseOrderDetailPage() {
  ...
  const { data: purchaseOrderList } = useQuery({
    ...
    queryFn: () => api.get<{ id: string }[]>('/purchase-orders'),
  });
  usePageUpDownNavigation(purchaseOrderList, id, (o) => `/purchase-orders/${o.id}`);
  ...
  const saveMutation = useMutation({
    mutationFn: (values: PurchaseOrderInput) => api.patch(`/purchase-orders/${id}`, values),
    onSuccess: () => {
  ...
  const convertMutation = useMutation({
    mutationFn: () => api.post<{ id: string }>(`/purchase-orders/${id}/convert`),
    onSuccess: (bill) => {
      queryClient.invalidateQueries({ queryKey: ['purchase-orders', id] });
      queryClient.invalidateQueries({ queryKey: ['purchase-orders'] });
      queryClient.invalidateQueries({ queryKey: ['inventory'] });
      toast.success('Converted to purchase bill — stock updated');
  ...
  return (
    <div className="flex flex-col gap-6">
      <Breadcrumb items={[{ label: 'Purchase Orders', href: '/purchase-orders' }, { label: order.supplier.name }]} />
      ...
      <Card className="max-w-2xl w-full mx-auto">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          {order.status === 'CONVERTED' && order.purchaseBill && (
            <p className="text-sm">
              Converted to purchase bill{' '}
              <Link href={`/purchase-bills/${order.purchaseBill.id}`} className="underline hover:text-foreground">
                {order.purchaseBill.billNumber ?? 'View'}
              </Link>
            </p>
          )}
          ...
          {order.status === 'OPEN' ? (
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {convertMutation.isPending ? 'Converting…' : 'Convert to purchase bill'}
                </Button>
                <Button
                  type="button"
                  variant="ghost"
