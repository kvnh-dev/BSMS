...
// Direct mirror of record-payment-sheet.tsx, against a PurchaseBill instead
// of an Invoice.
export function RecordSupplierPaymentSheet({
  ...
  const recordMutation = useMutation({
    mutationFn: (input: RecordPaymentInput) => api.post(`/purchase-bills/${purchaseBillId}/payments`, input),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['purchase-bills', purchaseBillId] });
      queryClient.invalidateQueries({ queryKey: ['purchase-bills', purchaseBillId, 'payments'] });
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent>
        ...
        <form id="record-supplier-payment-form" onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
          ...
          <FormField label="Reference (optional)">
            <Input placeholder="NEFT ref, cheque no…" {...form.register('reference')} />
          </FormField>
        </form>
        <SheetFooter>
          <Button type="submit" form="record-supplier-payment-form" disabled={recordMutation.isPending}>
